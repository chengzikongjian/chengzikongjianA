...
export function CharacterCard({ character, compact }: { character: CharacterItem; compact?: boolean }) {
  ...
  return (
    <article className={`character-card ${compact ? 'compact' : ''}`}>
      <div className="picture-orbit" aria-label={`${character.char}鐨勫疄鐗╁浘`}>
      ...
      <div className="card-actions">
        <button onClick={function() { speakCharacter(character) }} title="鎾斁璇婚煶">
          <Volume2 size={18} /> 鍚闊?
        </button>
        <button onClick={function() {
          ...
          <Play size={18} /> {showStroke ? '鏀惰捣绗旈『' : '鐪嬬瑪椤?}
        </button>
      </div>
      {showStroke && <div className="writer-box" ref={writerRef} aria-label="绗旈『鍔ㄧ敾" />}
    </article>
  )
}
